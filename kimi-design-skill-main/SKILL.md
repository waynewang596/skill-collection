...
- **Display fonts / hero titles**: read `references/font-system.md` when the task involves MiSans, a hero section title, display heading, brand landing title, or first-viewport product/title typography. MiSans is a narrow hero-title exception, not a replacement for UI typography tokens.
...
- **Logos and wordmarks**: read `references/logo-system.md` when the task involves brand marks, wordmarks, logo placeholders, app identity, header brand slots, or sub-brand lockups. Do not treat wordmarks as `24×24` UI icons.
...
- **Textured cover images**: read `references/visual-textures.md` when the task includes Card, Tile, Banner, Feature block, Empty State, or any media/cover slot that needs an abstract visual. Check the local texture pack manifest at `/Users/moonshot/kimi-texture-pack/manifest.json` before creating a new abstract cover.
...
3. **Font rules** (`font-system.md`) own the narrow MiSans exception for explicit hero section titles.
4. **Logo rules** (`logo-system.md`) own brand marks, wordmarks, logo placeholders, and sub-brand lockups.
5. **Component rules** (`components-web/*.md`, or the draft `components-mobile.md`) own component contracts: anatomy, allowed variants, sizes, states, component metrics, composition, and accessibility.
6. **Platform best practices** (`web-best-practices.md` or `components-mobile.md`) own page-level defaults: layout, density, information hierarchy, interaction completeness, and which component or variant is appropriate in context.
...
- Level B: font system, icon system, logo system, animation reference, visual texture reference, Web component references, draft Mobile component reference, plus Web page-level best practices.
