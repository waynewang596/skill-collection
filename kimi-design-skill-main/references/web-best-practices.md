...
This rhythm implements `principles.md` §Proximity And Grouping: related items should feel connected, while unrelated groups should separate clearly. Do not use equal spacing for unequal relationships; equal spacing makes users read items as peers.
...
Use spacing as the first grouping tool. Add borders, cards, dividers, or background fills only when spacing and alignment are not enough to express the relationship or when the component spec requires a framed surface.
...
Avoid decorative gradients, floating glow, glassmorphism, oversized shadows, and excessive card stacking. Prefer no shadow for static cards, panels, navigation, lists, and repeated content. Shadows are reserved for tokenized input focus or temporary floating elevation where mask, edge, spacing, and background contrast are not enough.
...
- If a new floating component is introduced and it lacks a mask, give it a `0.5px` stroke before considering alternatives. Add shadow only after the edge still fails to separate the surface clearly.
