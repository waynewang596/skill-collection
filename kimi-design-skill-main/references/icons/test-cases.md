...
### X5. Brand vs Function Collision

**Prompt**: "A dashboard shows: 'Kimi Model', 'Kimi+ Agents', 'KimiClaw Tools', 'K1.5 Engine'. What icon for each?"

**Expected**:
- Kimi Model → `TabKimiIcon` or generic `ModelIcon`
- Kimi+ Agents → `KimiPlusIcon`
- KimiClaw Tools → `KimiclawIcon`
- K1.5 Engine → `k15Icon` (or `k15_bIcon` / `k15_cIcon` depending on context)

**Why hell**: Tests whether the AI treats brand icons as semantic icons vs misusing them. `TabKimiIcon` is a navigation tab icon, not a generic "Kimi product" icon.

### X6. The "Analyze" Multiverse
