...
Avoid shadows by default. Prefer spacing, alignment, background contrast, and semantic borders to separate surfaces. Use tokenized shadows only when they clarify focus, elevation, or temporary floating state better than quieter treatments.
...
For non-chart interaction emphasis, prefer `color.brand.kimiDark`. Use it for focus rings, action text, selected interaction emphasis, and other product UI affordances unless a component spec defines a more specific semantic token. Reserve `color.status.kimiBlue` for data visualization and chart-derived color scales, not general UI interaction. If `color.brand.kimiDark` fails contrast on a fixed dark or tinted surface, preserve readability and record the missing inverse/contrast-safe interaction token instead of silently using blue.
...
Information hierarchy should match task priority. Do not give unrelated or lower-priority content the same visual weight as the primary object or action in the same local area. Establish reading order through position, typography, spacing, color, and component emphasis together rather than relying on any single visual cue.

## 4. Proximity And Grouping

Related information should sit closer together than unrelated information. Users infer structure from distance: labels belong near their fields, helper text and validation errors belong near the control they explain, actions belong near the object they affect, and metadata belongs near the content it qualifies.

Use spacing and alignment to establish groups before adding frames, cards, dividers, or background fills. Do not apply one uniform gap across elements with different relationships; equal spacing implies equal relationship. Larger gaps separate sections or object groups, while smaller gaps connect labels, values, controls, icons, and supporting text.

## 5. Typography For Reading And UI
...
Do not introduce new fonts, arbitrary sizes, negative letter spacing, or viewport-scaled type. The only current font exception is MiSans for explicit hero section titles; read `references/font-system.md` before using it. If a required text role is missing, record the gap and map it only after the design system source is confirmed.
...
- `focus-visible` (Web): use `color.brand.kimiDark`
...
Elevation should not automatically mean shadow. A floating or layered surface should first use mask, edge, spacing, or background contrast to define its boundary. Use shadow only when the component spec or focus state requires it, or when a visible edge is still insufficient on a complex background.
