...
For Web React chart implementations, prefer Recharts for standard line, bar, area, pie, donut, scatter, and composed charts unless the project framework does not support React/Recharts, the existing codebase already standardizes on another chart library, or the chart requires advanced rendering, very large datasets, custom layout, or specialized interactions.
